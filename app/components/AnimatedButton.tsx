...
export const AnimatedButton = ({ href, text, variant = 'primary' }: AnimatedButtonProps) => {
  ...
  const primaryClasses = "bg-[#0B615E] text-white hover:bg-[#A7FFEB] hover:text-[#0B615E]";
  const secondaryClasses = "border-2 border-[#0B615E] text-[#0B615E] hover:bg-[#0B615E] hover:text-white";
  ...
  return (
    ...
    >
      <div className={`flex-shrink-0 w-8 h-8 rounded-full border-2 flex items-center justify-center overflow-hidden transition-colors duration-200
        ${variant === 'primary' ? (isHovered ? 'border-[#0B615E]' : 'border-white') : (isHovered ? 'border-white' : 'border-[#0B615E]')}
      `}>
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"
           className={`feather feather-arrow-up-right w-4 h-4 transition-all duration-300
             ${variant === 'primary' ? (isHovered ? 'text-[#0B615E]' : 'text-white') : (isHovered ? 'text-white' : 'text-[#0B615E]')}
